...
const scanDirectories = () => {
	...
	return [
		...assetDirectories.map((directory) => walkDir(path.join(rootDirectory, directory))),
		getUpdatePluginXmls()
	];
};
...
Promise.all(scanDirectories())
	...
	.then((allAssets) => {
		...
		return Promise.all(
			...
			assetToCheckSums.reduce((accum: StringDictionary<string>, assetToChecksum) => {
	...
	.then((assetToCheckSum) => {
		...
		return Object.keys(assetToCheckSum)
			.filter((assetPath) => {
				...
				return !syncedAssets[assetKey] || syncedAssets[assetKey] !== assetToCheckSum[assetPath];
			})
			.map((changedAsset) => ({
				key: changedAsset,
